...
def  FederatedTrain(clients, model_type, outdir, time_window, max_flow_len, dataset_name, epochs='auto', steps='auto', training_mode = 'flad', weighted=False,optimizer='SGD',nr_experiments=EXPERIMENTS):
    ...
    while True:  # training epochs
        ...
        for client in client_subset:
            ...
            compileModel(client['model'], optimizer,'sparse_categorical_crossentropy')
            if client['epochs'] > max_epoch:
                max_epoch = client['epochs']
        ...
        for i in range(1, max_epoch+1):
            for client in client_subset:
                ...
                # If the client is selected, perform the local training with epoch = 1
            ...
            client_weighted_sum(client_subset, i - 1, weighted)
...
def assess_server_model(server_model, clients,update_clients=False, print_f1=False):
    ...
    for client in clients:
        ...
        Y_pred = np.squeeze(np.argmax(server_model.predict(X_val, batch_size=2048), axis=-1))
        print(Y_val)
        print(Y_pred)
        client_f1 = accuracy_score(Y_val, Y_pred)
        f1_val_list.append(client_f1)
        if update_clients == True:
            client['f1_val'] = client_f1
        if print_f1 == True:
...
def client_weighted_sum(clients, i, weighted=True):
    total = 0

    temp_model = clone_model(clients[0]['model'])
    temp_weights = temp_model.get_weights()

    weight_index = i % 3
    
    weight_avg_1 = np.zeros(temp_weights[weight_index * 2].shape)
    weight_avg_2 = np.zeros(temp_weights[weight_index * 2 + 1].shape)
    ...
    for client in clients:
        ...
        client_model = client['model']
        client_weights = client_model.get_weights()
        weight_avg_1 += client_weights[weight_index * 2] * avg_weight
        weight_avg_2 += client_weights[weight_index * 2 + 1] * avg_weight

    for client in clients:
        client_model = client['model']
        client_weights = client_model.get_weights()
        client_weights[weight_index * 2] = weight_avg_1 / total
        client_weights[weight_index * 2 + 1] = weight_avg_2 / total
